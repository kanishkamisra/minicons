...
class MaskedLMScorer(LMScorer):
    ...
    def prepare_text(self, text: Union[str, List[str]]) -> Union[str, List[str]]:
        ...
        for token_ids, attention_mask in zip(token_idx, attention_masks):
            ...
            masked_tensors.append((torch.stack(token_ids_masked_list), torch.stack(attention_masked_list), effective_token_ids, len(mask_indices)))
    ...
    def logprobs(self, batch: Iterable) -> Union[float, List[float]]:
        ...
        token_ids, attention_masks, effective_token_ids, lengths = list(zip(*batch))
        ...
        indices = list(itertools.chain.from_iterable([list(range(1,n+1)) for n in lengths]))
        with torch.no_grad():
